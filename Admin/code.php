...
if(isset($_POST['add_category_btn']))
{
    ...
    $cate_query_run = mysqli_query( $con ,$cate_query);
     ...
     if($cate_query_run)
     {
        ...
        redirect("add-category.php" , "Catégorie ajoutée avec succès ");
     }
     else
     {
...
else if(isset($_POST['update_category_btn']))
{
    ...
    if($udate_query_run)
    {
      ...
      redirect("edit-category.php?id=$category_id","Categorie Modifier successfuly " );
    }
    else
    {
  ...
  if($name != "" && $slug != "" && $description != "" )
  {
   ...
   (category_id,name,slug, small_description,description,original_price,selling_price,qty,meta_title,meta_description,meta_keywords,status,trending,image) 
      ...
      if($product_query_run)
      {
        ...
        redirect("Add-product.php" , "Product Added sucessfully ");
      }
      else
      {
  ...
  if($new_image != "")
    ...
    meta_description='$meta_description', meta_keywords='$meta_keywords', status='$status', trending='$trending',image='$update_filename'
     WHERE id='$product_id' ";
      ...
      redirect("edit-product.php?id=$product_id","Product Update successfuly " );
    }
    else
    {
...
else if(isset($_POST['add_brands_btn']))
{
  $id =$_POST['id'];
  $name = $_POST['name'];
  $slug = $_POST['slug'];
  ...
  if($name != "" && $slug != ""  )
  {
   ...
   $brands_query ="INSERT INTO brands (name ,slug ,status) VALUES ('$name','$slug','$status')";

       $brands_query_run = mysqli_query( $con ,$brands_query);

      if($brands_query_run)
      {
        
        redirect("Add-Brands.php" , "Brands Added sucessfully ");
      }
      else
      {
        redirect("Add-Brands.php" , "somthing wrong ");
      }
  }
  else
      {
        redirect("Add-Brands.php" , "Please try to complete forumlaire");
      }
}
